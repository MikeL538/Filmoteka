...
function populateModal(movieDetails) {
  ...
  // Funkcja do wyświetlania szczegółów filmu w modalu
  function showLibraryDetails(event) {
    ...
    if (event.target.classList.contains('btn-remove-from-queue')) {
      ...
      // Tutaj dodaj kod do automatycznego odświeżenia listy filmów "Kolejka"
      const queueList = JSON.parse(localStorage.getItem('queue')) || [];
      refreshMovieList(queueList, 'queue');
    ...
    if (event.target.classList.contains('btn-remove-from-watched')) {
      ...
      // Tutaj dodaj kod do automatycznego odświeżenia listy filmów "Obejrzane"
      const watchedList = JSON.parse(localStorage.getItem('watched')) || [];
      refreshMovieList(watchedList, 'watched');

      return;
    }

    function refreshMovieList(list, listName) {
      const libraryFilmsList = document.querySelector('.library-films__list');
      libraryFilmsList.innerHTML = '';

      list.forEach(movieId => {
        fetchMovieDetails(movieId, listName);
      });
    }
